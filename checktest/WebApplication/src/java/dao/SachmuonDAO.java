...
public class SachmuonDAO extends DAO {
    ...
    public boolean updateListSachmuon(List<Sachmuon325> dsSachmuon) {
        if ((dsSachmuon == null) || (dsSachmuon.size() == 0)) {
            return false;
        }
        boolean kq = false;
        String sql = "UPDATE `tblsachmuon` SET `trangthai` = '1', `ngaytra` = ? WHERE (`tblSachid` = ?);";
        try {
            for (Sachmuon325 i : dsSachmuon) {
                ps = con.prepareStatement(sql);
    ...
    public boolean updateIdphatListStm(List<Sachmuon325> dsSachtramuon) {
        if ((dsSachtramuon == null) || (dsSachtramuon.size() == 0)) {
            return false;
        }
        ...
        try {
            ...
            for (Sachmuon325 i : dsSachtramuon) {
