...
<%
  ...
  if (nv == null) {
    ...
    if (kq) {
        ...
        int idpm=0;
        if(listTramuon.size() > 0) idpm = listTramuon.get(0).getPhieumuon().getId();
        String mavachPhieumuon = "";
        if(list0.size() > 0) mavachPhieumuon=list0.get(0).getPhieumuon().getMavachPhieumuon();
        else if(listTramuon.size()>0) mavachPhieumuon=listTramuon.get(0).getPhieumuon().getMavachPhieumuon();
